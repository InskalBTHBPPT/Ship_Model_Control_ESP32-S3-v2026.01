            ...
            while self.running:
                ...
                if available > 0:
                    ...
                    while '\n' in self.text_buffer:
                        ...
                        line = self.text_buffer[:line_end].strip()
                        ...
                        try:
                            # Parse CSV line
                            # Format: timestamp,latitude,longitude,speedMps,Calc_deg_servo_1,Calc_deg_servo_2,roll,pitch,yaw,zigzag_yaw,rpm_prop_1,rpm_prop_2,battery_1,battery_2,mode_auto
                            parts = line.split(',')
                            
                            if len(parts) != 15:
                                self.error_occurred.emit(f"[ERR] Jumlah field tidak sesuai: {len(parts)} != 15\n")
                                continue
                            
                            # Parse dan konversi data
                            # 3 double: timestamp, latitude, longitude (tetap float)
                            timestamp = float(parts[0])
                            latitude = float(parts[1])
                            longitude = float(parts[2])
                            
                            # 8 int16_t dengan skala × 100 (dibagi 100 untuk mendapatkan nilai asli)
                            speedMps = int(parts[3]) / 100.0
                            Calc_deg_servo_1 = int(parts[4]) / 100.0
                            Calc_deg_servo_2 = int(parts[5]) / 100.0
                            roll = int(parts[6]) / 100.0
                            pitch = int(parts[7]) / 100.0
                            yaw = int(parts[8]) / 100.0
                            zigzag_yaw = int(parts[9]) / 100.0
                            
                            # 2 int16_t untuk RPM (integer, tidak perlu dibagi)
                            rpm_prop_1 = int(parts[10])
                            rpm_prop_2 = int(parts[11])
                            
                            # 2 int16_t untuk battery dengan skala × 100 (dibagi 100)
                            battery_1 = int(parts[12]) / 100.0
                            battery_2 = int(parts[13]) / 100.0
                            
                            # 1 uint8_t untuk mode_auto (integer)
                            mode_auto = int(parts[14])
                            
                            # Format sebagai CSV string untuk display dan logging (format yang sama dengan sebelumnya)
                            csv_line = (
                            ...
                            self.error_occurred.emit(f"[ERR] Gagal parse CSV: {e} - Line: {line[:50]}...\n")
                            continue
