...
- Fitur: map tracking, live indicators, time-series, logging CSV, analyze,
  Home Points, Send Way Points, Shutdown Mini PC, u/v Live + log.
...
TELEMETRY_LOG_HEADER = (
    "timestamp (s),latitude (°),longitude (°),speedMps (m/s),u (m/s),v (m/s),Calc_deg_servo_1 (°),Calc_deg_servo_2 (°),"
...
class EnuBodyVelocityTracker:
    ...
    def update(self, timestamp: float, lat: float, lon: float, yaw_deg: float) -> tuple[float, float]:
        if abs(lat) < _GPS_INVALID_ABS and abs(lon) < _GPS_INVALID_ABS:
            self.have_prev = False
            self.x_dot = 0.0
            self.y_dot = 0.0
            return 0.0, 0.0
        ...
        x = math.radians(lon - self.lon0) * _ENU_R_M * self.cos_lat0
        y = math.radians(lat - self.lat0) * _ENU_R_M
        ...
        self.prev_t = timestamp
        if dt < _VEL_MIN_DT_S or dt > _VEL_MAX_DT_S:
            self.prev_x = x
            self.prev_y = y
            self.x_dot = 0.0
            self.y_dot = 0.0
            return 0.0, 0.0
        x_raw = (x - self.prev_x) / dt
        y_raw = (y - self.prev_y) / dt
        self.prev_x = x
        self.prev_y = y
        self.x_dot = _VEL_LPF_ALPHA * self.x_dot + (1.0 - _VEL_LPF_ALPHA) * x_raw
        self.y_dot = _VEL_LPF_ALPHA * self.y_dot + (1.0 - _VEL_LPF_ALPHA) * y_raw
...
def _build_telemetry_log_line(
    ...
    return (
        ...
        f"{speed:.2f},{u_surge:.3f},{v_sway:.3f},"
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.enu_body_vel = EnuBodyVelocityTracker()
        ...
        self.u_surge_label = QLabel("0.00 m/s", self)
        self.u_surge_label.setStyleSheet(value_style)
        self.v_sway_label = QLabel("0.00 m/s", self)
        self.v_sway_label.setStyleSheet(value_style)
    ...
    def update_indicators(self, yaw: float, heading_setpoint: float, heading_error: float,
                          ...
                          mini_pc_link: int = 0, u_surge: float = 0.0, v_sway: float = 0.0):
        ...
        try:
            self.u_surge_label.setText(f"{u_surge:.2f} m/s")
            self.v_sway_label.setText(f"{v_sway:.2f} m/s")
        except Exception:
            pass
    ...
    def set_home_point_from_serial(self):
        ...
        self.enu_body_vel.set_origin(self.latest_serial_lat, self.latest_serial_lon)
    ...
    def disconnect_serial(self):
        ...
        try:
            ...
            self.enu_body_vel.reset()
            if hasattr(self, "u_surge_label"):
                self.u_surge_label.setText("0.00 m/s")
                self.v_sway_label.setText("0.00 m/s")
        ...
        try:
            ...
            while b"\n" in self.serial_buffer:
                ...
                try:
                    ...
                    u_surge, v_sway = self.enu_body_vel.update(
                        timestamp, gps_lat, gps_lon, heading
                    )
                except Exception:
                    continue
