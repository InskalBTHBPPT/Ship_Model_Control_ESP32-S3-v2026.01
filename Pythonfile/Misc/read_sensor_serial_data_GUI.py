...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QComboBox, QPushButton, QLabel, QSplitter, QLineEdit, QPlainTextEdit
)
...
class SerialReaderThread(QThread):
    ...
    def __init__(self, port, baud_rate, csv_path, csv_header=""):
        ...
        self.binary_buffer = b''  # Buffer untuk binary data (untuk menangani data yang terpotong)
        self.struct_size = 47  # Ukuran struct DatatoSend: 3*8 (double) + 11*2 (int16_t) + 1*1 (uint8_t) = 24 + 22 + 1 = 47 bytes
        # Format struct: '<' = little endian, 3d = 3 double, 11h = 11 int16_t, 1B = 1 uint8_t
        # Urutan: timestamp, latitude, longitude, speedMps, Calc_deg_servo_1, Calc_deg_servo_2, roll, pitch, yaw, zigzag_yaw, rpm_prop_1, rpm_prop_2, battery_1, battery_2, mode_auto
        self.struct_format = '<3d11hB'  # Little endian format: 3 double + 11 int16_t + 1 uint8_t
    ...
    def run(self):
        ...
        try:
            ...
            while self.running:
                ...
                # Baca binary data dari serial
                available = self.ser.in_waiting
                if available > 0:
                    chunk = self.ser.read(available)
                    self.binary_buffer += chunk
                    
                    # Proses semua paket lengkap yang ada di buffer
                    while len(self.binary_buffer) >= self.struct_size:
                        # Ambil satu paket (76 bytes)
                        packet = self.binary_buffer[:self.struct_size]
                        self.binary_buffer = self.binary_buffer[self.struct_size:]
                        
                        try:
                            # Verify packet size
                            if len(packet) != self.struct_size:
                                self.error_occurred.emit(f"[ERR] Packet size mismatch: {len(packet)} != {self.struct_size}\n")
                                continue
                            
                            # Unpack binary data (little endian)
                            unpacked = struct.unpack(self.struct_format, packet)
                            
                            # Extract data sesuai urutan struct
                            # 3 double: timestamp, latitude, longitude
                            timestamp = unpacked[0]
                            latitude = unpacked[1]
                            longitude = unpacked[2]
                            
                            # 11 int16_t: 9 dengan skala 100 (dibagi 100), 2 RPM integer (tidak dibagi)
                            speedMps = unpacked[3] / 100.0
                            Calc_deg_servo_1 = unpacked[4] / 100.0
                            Calc_deg_servo_2 = unpacked[5] / 100.0
                            roll = unpacked[6] / 100.0
                            pitch = unpacked[7] / 100.0
                            yaw = unpacked[8] / 100.0
                            zigzag_yaw = unpacked[9] / 100.0
                            rpm_prop_1 = unpacked[10]  # Integer 0..30000 (tidak perlu dibagi)
                            rpm_prop_2 = unpacked[11]  # Integer 0..30000 (tidak perlu dibagi)
                            battery_1 = unpacked[12] / 100.0
                            battery_2 = unpacked[13] / 100.0
                            
                            # 1 uint8_t untuk mode_auto (integer)
                            mode_auto = unpacked[14]
                            
                            # Format sebagai CSV string untuk display dan logging
                            csv_line = (
                                f"{timestamp:.3f},{latitude:.6f},{longitude:.6f},"
                                f"{speedMps:.2f},{Calc_deg_servo_1:.2f},{Calc_deg_servo_2:.2f},"
                                f"{roll:.2f},{pitch:.2f},{yaw:.2f},{zigzag_yaw:.2f},"
                                f"{rpm_prop_1:.0f},{rpm_prop_2:.0f},{battery_1:.2f},{battery_2:.2f},{mode_auto}"
                            )
                            
                            # Emit data ke GUI
                            self.data_received.emit(csv_line + "\n")
                            
                            # Simpan ke buffer (batch writing untuk mengurangi I/O overhead)
                            if self.csv_file:
                            ...
                            self.error_occurred.emit(f"[ERR] Gagal unpack binary: {e}\n")
                            continue
...
class SerialMonitorGUI(QMainWindow):
    ...
    def connect_serial(self):
        ...
        # Buat thread untuk membaca serial
