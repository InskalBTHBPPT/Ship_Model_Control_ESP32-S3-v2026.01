# Cpp_ReadWriteSerial-1.1-NED-beta

Versi **1.1-NED-beta** dari mini-PC serial bridge — clone dari `Cpp_Files/Cpp_ReadWriteSerial-1.1-ENU-beta` dengan transformasi **lat/lon → NED (internal)**.
...
| Varian ENU | `Cpp_Files/Cpp_ReadWriteSerial-1.1-ENU-beta` |
| Penerus | `Cpp_Files/Cpp_ReadWriteSerial-1.2-NED-beta` |
...
### Frame lokal NED (hanya internal)

Lat/lon telemetry diubah ke meter relatif origin. **Tidak** dicetak ke stdout dan **tidak** dikirim ke serial.
...
| Origin | Nilai |
|--------|-------|
| Default (home belum ada / `[WP] Home: <none>`) | `-7.2867106, 112.7958064` |
| Setelah `[WP] Home: lat, lon` | koordinat home itu (`0, 0` di NED) |

Status origin dan last `x,y` hanya muncul di **stderr**.
...
```powershell
cd "Cpp_Files\Cpp_ReadWriteSerial-1.1-NED-beta"
