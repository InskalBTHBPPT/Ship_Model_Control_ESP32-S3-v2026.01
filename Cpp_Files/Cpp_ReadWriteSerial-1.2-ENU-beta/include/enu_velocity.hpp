/**
 * @file enu_velocity.hpp
 * @brief Kinematika ENU: posisi x,y (m) → ẋ,ẏ, |V|, surge u, sway v (m/s).
 *
...
// x, y = posisi ENU (m) relatif origin.
...
struct EnuKinematics {
  LocalXy xy{};        // x=East (m), y=North (m) — posisi / perpindahan
  double x_dot = 0.0;  // ẋ East  (m/s)
  double y_dot = 0.0;  // ẏ North (m/s)
...
// ẋ,ẏ peta ENU → u,v badan. ψ dari Utara, naik ke timur (kompas).
...
struct EnuVelocityTracker {
  ...
  EnuKinematics update(double timestamp_s, double lat, double lon, double yaw_deg,
    ...
    if (!out.gps_ok) {
      reset();
      enu_vel_to_body(0.0, 0.0, out.psi, out.u, out.v);
      return out;
    }
    ...
    enu_vel_to_body(out.x_dot, out.y_dot, out.psi, out.u, out.v);
    out.vel_ok = true;
    return out;
  }
};
