/**
 * @file main.cpp
 * @brief Cpp_ReadWriteSerial-1.2-ENU-beta — ENU posisi + kecepatan u,v
 *
 * Clone 1.1-ENU-beta. Internal: x,y (m), ẋ,ẏ (m/s), surge u / sway v (m/s).
 * CSV stdout dan serial TX tidak berubah. Rudder belum memakai NMPC.
 */

#include "enu_velocity.hpp"
...
int main(int argc, char **argv) {
  ...
  EnuVelocityTracker vel_tracker;
  EnuKinematics kin{};
  ...
  std::cerr << "\n[INFO] Selesai. Baris valid: " << valid_lines
            ...
            << " | last ENU x=" << kin.xy.x << " y=" << kin.xy.y
            << " u=" << kin.u << " v=" << kin.v << " m/s\n";
  return 0;
}
