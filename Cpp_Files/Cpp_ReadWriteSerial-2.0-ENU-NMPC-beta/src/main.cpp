 ...
 * gyro_z diabaikan; r dari yaw_rate. v = 0. ψ = π/2 − yaw_kompas.
 */
...
// yaw kompas [deg], 0=Utara → psi NMPC [rad], 0=Timur
double compass_yaw_to_nmpc_psi(double yaw_deg) {
  return wrap_pi((M_PI / 2.0) - (yaw_deg * (M_PI / 180.0)));
}

// yaw_rate kompas [deg/s] → r nondim (tanda terbalik karena psi = pi/2 - yaw)
...
int main(int argc, char **argv) {
  ...
  std::cerr << std::setprecision(7)
            ...
            << " (menunggu [WP] Home)\n";
  std::cerr << "[INFO] v=0 | r dari yaw_rate | psi = pi/2 - yaw | gyro_z abaikan\n";
